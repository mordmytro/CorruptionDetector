...
def build_classification_model():
    ...
    def test_coef(coef=0.5):
        ...
        print(
            f"Correct predictions: {round(100 * total_correct/total_predictions if total_predictions != 0 else 0, 5)}% ({total_correct}/{total_predictions})"
        )
        print(
            f"Incorrect predictions: {round(100 * total_incorrect/total_predictions if total_predictions != 0 else 0, 5)}% ({total_incorrect}/{total_predictions})"
        )
        print(
            f"Not corrupted predicted correct: {round(100 * good_predicted_good/total_good if total_predictions != 0 else 0, 5)}% ({good_predicted_good}/{total_good})"
        )
        print(
            f"Not corrupted predicted incorrect: {round(100 * good_predicted_bad/total_good if total_predictions != 0 else 0, 5)}% ({good_predicted_bad}/{total_good})"
        )
        print(
            f"Corrupted predicted correct: {round(100 * bad_predicted_bad/total_bad if total_predictions != 0 else 0, 5)}% ({bad_predicted_bad}/{total_bad})"
        )
        print(
            f"Corrupted predicted incorrect: {round(100 * bad_predicted_good/total_bad if total_predictions != 0 else 0, 5)}% ({bad_predicted_good}/{total_bad})"
        )
        print("--------------------------------------")
